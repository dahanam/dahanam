# Hi, I'm Dahana 👋

Computer Science student at Kean University, graduating May 2026. I work on data science, AI/ML, and full-stack web projects — including NSF-funded research on student success in CS.

---

## 🔬 Research

**Cougar Pathway to Success** — NSF Awards #1928452, #2129795, #2345334 + Sloan Foundation  
Longitudinal analysis of 3,191 student records linking co-curricular engagement to professional outcomes. Built regression models (R²=0.53) and segmented 6 engagement tiers showing offer rates rising from 2% to 41.6% at 7+ activities. Co-authoring paper for ACM journal.

**Equity in Software** — NSF #2345334  
Supported curriculum development and student data collection for broadening participation in CS.

---

## 💼 Experience

- **NSF Cougar Pathway to Success** — Data Science Research Assistant (2025–2026)
- **CPC Integrated Health** — AI/ML Developer, built EHR chatbot with semantic RAG pipeline (2025–2026)
- **NSF Equity in Software** — Research Assistant (2023–2026)
- **Girls Who Code** — Club Leader (80+ members, IBM/Lockheed/NASA partnerships)

---

## 🛠 Projects

| Project | Description | Tech |
|---|---|---|
| [Cyberbullying Detection](https://github.com/dahanam/Cyberbullying-Detection) | BERT, SentenceTransformers, GPT-4, clustering | Python, TF, HuggingFace |
| [AI vs Real Image Classifier](https://github.com/dahanam/AI-vs-Real-Image-Classifier) | ResNet-50 & VGG-16, Monte Carlo uncertainty | Python, PyTorch |
| [EHR Manual QA Chatbot](https://github.com/dahanam/EHR-Manual-QA-Chatbot) | Semantic RAG, multi-provider LLM, intent classification | Python, OpenAI, Claude |
| [Foundation Recommender](https://github.com/dahanam/AI-Powered-Digital-Foundation-Recommender-System) | Webcam skin tone detection + K-Means clustering | Python, OpenCV |
| [Stock Market Dashboard](https://github.com/dahanam/Stock-Market-Dashboard) | Interactive charts, parallel ETL pipeline | PHP, JS, Python |
| [Medical & Loan Dashboard](https://github.com/dahanam/Medical-Cost-Loan-Data-Visualization-Dashboard) | Dynamic visualizations, AJAX filters | PHP, JS, Google Charts |
| [CTF Challenge Assistant](https://github.com/dahanam/CTF-Challenge-Solving-Assistant) | GPT-4o agent with tool use, +50% solve rate | Python, OpenAI |
| [Java Data Structures](https://github.com/dahanam/Java-Data-Structures) | Heap, queue, GenderMag evaluation | Java |

---

## 🧰 Skills

Python · SQL · Java · JavaScript · PHP · TensorFlow · PyTorch · HuggingFace · scikit-learn · pandas · React · Git

---

## 📫 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-dahanam-blue?style=flat&logo=linkedin)](https://linkedin.com/in/dahana-moz-ruiz)

---

## 📄 Resume & Google Scholar Page

[![LinkedIn](https://img.shields.io/badge/LinkedIn-dahana--moz--ruiz-blue?style=flat&logo=linkedin)](https://docs.google.com/document/d/1uwpBtiX3AQEg1iWoM8r9344KufzC555Kv9Qo6xco_10/edit?usp=sharing)
[![Google Scholar](https://img.shields.io/badge/Google%20Scholar-D.%20Moz%20Ruiz-lightgrey?style=flat&logo=google-scholar)](https://scholar.google.com/citations?user=fyMMHOQAAAAJ&hl=en)
